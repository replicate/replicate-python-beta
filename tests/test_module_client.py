...
def test_base_url_option() -> None:
    ...
    assert replicate.client._client.base_url == URL("https://api.replicate.com/v1/")

    replicate.base_url = "http://foo.com"

    assert replicate.base_url == URL("http://foo.com")
    assert replicate.client._client.base_url == URL("http://foo.com")


def test_timeout_option() -> None:
    assert replicate.timeout == replicate.DEFAULT_TIMEOUT
    assert replicate.client._client.timeout == replicate.DEFAULT_TIMEOUT

    replicate.timeout = 3

    assert replicate.timeout == 3
    assert replicate.client._client.timeout == 3


def test_max_retries_option() -> None:
    assert replicate.max_retries == replicate.DEFAULT_MAX_RETRIES
    assert replicate.client._client.max_retries == replicate.DEFAULT_MAX_RETRIES

    replicate.max_retries = 1

    assert replicate.max_retries == 1
    assert replicate.client._client.max_retries == 1
